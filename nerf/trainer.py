...
from loss import mse, psnr, ssim
...
class Trainer(object):
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if not self.no_log:
                # collect stats mettre un if pour les tests de temps
                if self.opt.loss == "sat":
            ...
            if not self.no_log:
                if self.local_rank == 0:
                    ...
                    if self.scheduler_update_every_step:
                        if self.opt.loss == "sat":
                            if self.local_step % 4096 == 0: # for each batch
    ...
    def train_step(self, data):
        ...
        if 'ts' in data:
            if self.opt.loss == "sat":
                after_N_epoch=self.epoch>self.opt.first_beta_epoch # param à twister ...
            ...
            for data in loader:
                ...
                if self.local_rank == 0:
                    ...
                    if for_dsm:
                        ...
                        # save as png but radiometrt may not be true for rgb
                        if self.opt.color_space == 'linear':
    ...
    def eval_step(self, data):
        ...
        if 'ts' in data:
            ...
            #
            if self.opt.loss == "sat":
                loss, ssim_, psnr_, mse_ = self.criterion(outputs, gt_rgb, after_n_epoch=True,B=B, H=H, W=W, training=False) # [B, N, 3] --> [B, N]
                loss = loss.mean()
            else:
